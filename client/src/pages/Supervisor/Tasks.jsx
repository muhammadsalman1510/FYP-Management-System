...
/*
  SUPERVISOR — TASKS
  Tabs:
    1. Create Task   — assign to "All My Students" or specific students
    2. All Tasks     — view tasks created by this supervisor
    3. Submissions   — view student submissions, accept/reject with feedback
  TODO (Backend):
    POST /api/supervisor/tasks
    GET  /api/supervisor/tasks
    GET  /api/supervisor/tasks/submissions
    PUT  /api/supervisor/tasks/submissions/:id/review
*/
...
const SupervisorTasks = () => {
  ...
  // TODO (Backend): GET /api/supervisor/students (only assigned students)
  const myStudents = [
    ...
    { id: 5, name: 'Usman Tariq',     rollNumber: 'F2021001006' },
  ];

  const [tasks, setTasks] = useState([
    { id: 1, title: 'Literature Review', instructions: 'Find and summarize 10 research papers.', openDate: '2025-11-02', dueDate: '2025-11-13', assignTo: 'all_my_students', assignedStudents: [], submissionsCount: 3 },
    { id: 2, title: 'Progress Report',   instructions: 'Submit your mid-semester progress.', openDate: '2025-11-15', dueDate: '2025-11-25', assignTo: 'specific', assignedStudents: [1, 2], submissionsCount: 1 },
  ]);
  ...
  // Create task form
  const emptyTask = { title: '', instructions: '', openDate: '', dueDate: '', assignTo: 'all_my_students', assignedStudents: [], attachFile: null };
  ...
  const handleCreateTask = (e) => {
    e.preventDefault();
    // TODO (Backend): POST /api/supervisor/tasks
  ...
  const submitReview = () => {
    ...
    // TODO (Backend): PUT /api/supervisor/tasks/submissions/:id/review
    setSubmissions(prev =>
          ...
          <div className="d-flex border-bottom">
            {[
              ...
              <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                ...
                style={{ color: activeTab === tab.key ? '#3c50e0' : '#6c757d', borderBottom: activeTab === tab.key ? '2px solid #3c50e0' : '2px solid transparent', fontSize: '0.9rem' }}>
                {tab.label}
              </button>
            ))}
          </div>
            ...
            <form onSubmit={handleCreateTask}>
              <div className="row g-3">
                ...
                <div className="col-12">
                  ...
                  <textarea name="instructions" value={taskForm.instructions} onChange={handleTaskFormChange} className="form-control" rows={4} placeholder="Describe the task requirements..." required />
                </div>
                ...
                <div className="col-12">
                  <label className="form-label fw-medium text-dark small">Attach File <span className="text-muted fw-normal">(Optional)</span></label>
                ...
                <tbody>
                  {tasks.map((task, index) => (
                    <tr key={task.id}>
                      ...
                      <td className="px-4 py-3">
                        {task.assignTo === 'all_my_students' ? (
                          <span className="badge bg-primary rounded-pill px-3 small">All My Students</span>
                        ) : (
                ...
                <tbody>
                  {submissions.map(sub => (
                    <tr key={sub.id}>
                      ...
                      <td className="px-4 py-3">
                        <span className={`badge rounded-pill px-3 py-2 ${getStatusBadge(sub.status)}`} style={{ fontSize: '0.72rem' }}>
          ...
          <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: '480px' }}>
            <div className="modal-content border-0 shadow">
              ...
              <div className="modal-body px-4 py-4">
                ...
                <textarea className="form-control" rows={3} placeholder="Provide feedback for the student..."
